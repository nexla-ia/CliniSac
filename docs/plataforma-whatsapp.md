# CliniSac — Documentação dos recursos "estilo WhatsApp"
...
**Render:** ao lado do horário, só nas mensagens que NÃO são do cliente (`!isCliente`) e que não falharam/foram apagadas. Modelo simplificado — **sempre 2 tiques** (`CheckCheck`), muda só a cor: cinza = enviado (padrão, `read_at` vazio), azul (`#53BDEB`, a cor do WhatsApp) = lido (`read_at` setado). Não tem estado de 1 tique nem tique cinza duplo "entregue" — `delivered_at` existe na coluna (pra guardar o dado/horário, se um dia quiser usar), mas não influencia o ícone hoje. Tooltip mostra o horário exato de leitura. Atualiza ao vivo pelo mesmo realtime UPDATE que já existia (apagada/reação/enquete).
...
_Gerado a partir do trabalho feito no CliniSac (clinimag). Cada item tem o commit correspondente no histórico do repo._
