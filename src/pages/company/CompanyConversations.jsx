...
// Alterna o 9º dígito do celular BR — usado só como FALLBACK de envio quando
// o Evolution diz "número não existe" (bagunça do nono dígito: DDD que exige o
// 9 recebe normal, mas recusa o envio sem ele). 12 díg (55+DDD+8) ganha o 9;
// 13 díg (55+DDD+9+8) perde o 9. Fora desse padrão BR, devolve null (sem retry).
function brToggle9(numero) {
  const d = String(numero || '').replace(/@.*/, '').replace(/\D/g, '')
  if (!d.startsWith('55')) return null
  if (d.length === 12) return `55${d.slice(2, 4)}9${d.slice(4)}@s.whatsapp.net`
  if (d.length === 13 && d[4] === '9') return `55${d.slice(2, 4)}${d.slice(5)}@s.whatsapp.net`
  return null
}
...
export default function CompanyConversations() {
  ...
  const [crmSubOpen, setCrmSubOpen]       = useState(false)  // submenu "Enviar para etapa do CRM" dentro do ⋯
  ...
  useEffect(() => { setCrmMenuOpen(false); setMoreMenuOpen(false); setCrmSubOpen(false) }, [selected?.session_id])
  // Recolhe o submenu do CRM quando o menu ⋯ fecha
  useEffect(() => { if (!moreMenuOpen) setCrmSubOpen(false) }, [moreMenuOpen])
  ...
  async function handleSend() {
    ...
    try {
      ...
      // Monta o corpo do POST pro n8n. O número de SAÍDA (session_id/phone) é
      // parametrizado pra permitir o retry com o 9 alternado — o que a gente
      // grava no banco continua no formato canônico (canonSid, sem 9).
      const buildBody = (sidJid) => JSON.stringify({
        message: sentText,
      ...
      // Uma tentativa de envio (com timeout próprio). Devolve o texto e o motivo
      // de falha (null = sucesso).
      const postOnce = async (sidJid) => {
        ...
        try {
          const r = await fetch(webhookUrl, {
            ...
            headers: { 'Content-Type': 'application/json' },
            signal: ctrl.signal,
            body: buildBody(sidJid),
          })
          const text = await r.text().catch(() => '')
          // Ruído de infra do n8n (ex.: lock do SQLite no "Respond to Webhook")
          // não é falha de entrega — ignora em silêncio (sem aviso, sem falhou).
          if (isN8nInfraNoise(text)) return { text: '', reason: null }
          if (!r.ok) throw new Error('HTTP ' + r.status)
          return { text, reason: detectSendError(text) }
        } finally {
          clearTimeout(timer)
        }
      }
      ...
      // Falha de CONEXÃO = a requisição não chegou no n8n (fetch rejeitou por
      // rede). NÃO inclui timeout/abort nem erro HTTP — nesses o n8n pode ter
      // recebido, então re-tentar arriscaria enviar 2x.
      const isConnFailure = (e) =>
        (e && e.name === 'TypeError') ||
        /failed to fetch|networkerror|err_network|load failed/i.test(String(e?.message || ''))

      // Envia; se a requisição NÃO chegar no n8n (falha de conexão), re-tenta
      // até 3x com backoff leve. Timeout/erro HTTP não re-tenta (risco de 2x).
      const sendWithRetry = async (sidJid) => {
        const MAX = 3
        for (let attempt = 1; ; attempt++) {
          try {
            return await postOnce(sidJid)
          } catch (e) {
            if (!isConnFailure(e) || attempt >= MAX) throw e
            await sleep(1000 * attempt)  // 1s, depois 2s
          }
        }
      }

      // Marca a última mensagem própria com um flag (falhou = vermelho "não
      // entregue"; incerto = laranja "entrega não confirmada").
      const markMsg = (patch) => setMessages(prev => {
        for (let i = prev.length - 1; i >= 0; i--) {
          if (prev[i].type === 'atendente' && prev[i].content === mensagemPayload) {
            const n = [...prev]; n[i] = { ...n[i], ...patch }; return n
          }
        }
        return prev
      })
      const markFalhou = () => markMsg({ falhou: true })
      const markIncerto = () => markMsg({ incerto: true })

      // Sucesso: n8n devolve instancia / mensagem / id_mensagem (uma por linha).
      const applyOk = async (text) => {
        ...
        // id_mensagem nunca tem espaço — se tiver, o corte saiu errado.
        ...
        const { data: row } = await supabase
          ...
          .maybeSingle()
        if (row?.id) {
          supabase.from('mensagens_geral').update({ id_mensagem: msgId }).eq('id', row.id).then(() => {})
          setMessages(prev => prev.map(m => m.id === row.id ? { ...m, id_mensagem: msgId } : m))
        }
      }

      // Fire-and-forget: tenta enviar (com auto-retry em falha de conexão). Se o
      // Evolution disser "número não existe" (nono dígito), reenvia com o 9
      // alternado. Aviso em 2 níveis: vermelho quando REALMENTE não foi (sem
      // conexão / Evolution recusou); laranja quando só faltou a confirmação
      // (timeout / resposta perdida — provavelmente foi, não dá pra ter certeza).
      ;(async () => {
        try {
          let res = await sendWithRetry(canonSid)
          if (res.reason && /n[aã]o existe/i.test(res.reason)) {
            const alt = brToggle9(canonSid)
            if (alt && alt !== canonSid) res = await sendWithRetry(alt)
          }
          if (res.reason) {
            setToast({
              message: `⚠️ Mensagem NÃO entregue no WhatsApp: ${res.reason}. Confira o número e tente de novo.`,
              color: '#DC2626',
            })
            setTimeout(() => setToast(null), 8000)
            markFalhou()
          ...
          await applyOk(res.text)
        } catch (e) {
          console.warn('webhook envio:', e)
          if (isConnFailure(e)) {
            // Não chegou no n8n mesmo após re-tentar → realmente não enviou.
            setToast({
              message: '⚠️ Mensagem NÃO enviada — sem conexão com o servidor (já tentei reenviar). Tente de novo em instantes.',
              color: '#DC2626',
            })
            setTimeout(() => setToast(null), 8000)
            markFalhou()
          } else {
            // Timeout / erro HTTP → o n8n pode ter recebido; provavelmente foi.
            setToast({
              message: 'ℹ️ Entrega não confirmada. A mensagem ficou salva e provavelmente foi enviada — confira no WhatsApp antes de reenviar (pra não duplicar).',
              color: '#D97706',
            })
            setTimeout(() => setToast(null), 8000)
            markIncerto()
          ...
          <>
                ...
                return (
                  <>
                    ...
                    <div className="chat-header-info" style={{ flex: 1, minWidth: 0 }}>
                      ...
                      {/* Uma linha só, com "..." no fim em vez de quebrar — antes
                          era um flex row de 2 spans que "brigavam" por espaço e
                          embolava telefone+contador em 2-3 linhas no celular. */}
                      <div style={{ fontSize: 12, color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {headerName && <span style={{ fontFamily: 'monospace' }}>{selected.phone}</span>}
                        {headerName && !loadingMsgs && ' · '}
                        {!loadingMsgs && `${messages.length} mensagem(ns)`}
                      </div>
                    </div>
                  </>
                )
                      ...
                      {moreMenuOpen && createPortal(
                        <>
                          ...
                          <div style={{
                            ...
                            {/* Enviar para etapa do CRM (submenu que expande as etapas) */}
                            {(() => {
                              const crmKey = normalizeBRDigits(selected.phone)
                              const lead = crmLeadMap[crmKey]
                              const curStage = lead ? crmStages.find(s => s.id === lead.stage_id) : null
                              return (
                                <>
                                  <button style={mi} onMouseEnter={hov} onMouseLeave={out}
                                    onClick={() => setCrmSubOpen(v => !v)}>
                                    <Kanban size={15} color={lead ? '#16A34A' : '#4F46E5'} />
                                    <span style={{ flex: 1 }}>{lead ? `No CRM: ${curStage?.nome || 'etapa'}` : 'Enviar para etapa do CRM'}</span>
                                    <ChevronRight size={14} color="var(--text-muted)" style={{ transform: crmSubOpen ? 'rotate(90deg)' : 'none', transition: 'transform .15s' }} />
                                  </button>
                                  {crmSubOpen && (
                                    <div style={{ margin: '2px 0 6px 12px', paddingLeft: 8, borderLeft: '2px solid #EEF2F7' }}>
                                      <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', padding: '4px 8px 5px' }}>
                                        {lead ? 'Mover para a etapa' : 'Adicionar na etapa'}
                                      </div>
                                      {crmStages.map(s => {
                                        const active = curStage?.id === s.id
                                        return (
                                          <button key={s.id} disabled={crmSaving}
                                            onClick={() => { handleAddToCrm(selected, s.id); setMoreMenuOpen(false) }}
                                            onMouseEnter={e => { if (!active) e.currentTarget.style.background = '#F1F5F9' }}
                                            onMouseLeave={e => { if (!active) e.currentTarget.style.background = 'transparent' }}
                                            style={{
                                              display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '7px 8px',
                                              border: 'none', borderRadius: 6, background: active ? '#EFF6FF' : 'transparent',
                                              cursor: crmSaving ? 'default' : 'pointer', fontSize: 12.5, fontWeight: active ? 700 : 500,
                                              color: '#0F172A', textAlign: 'left',
                                            }}>
                                            <span style={{ width: 9, height: 9, borderRadius: '50%', background: s.cor || '#64748B', flexShrink: 0 }} />
                                      ...
                                      {crmStages.length === 0 && (
                                        <div style={{ padding: '6px 8px', fontSize: 12, color: 'var(--text-muted)' }}>Nenhuma etapa no CRM ainda.</div>
                                      )}
                                    </div>
                                  )}
            ...
            {!isClosed && (
              <div style={{ padding: '12px 18px calc(16px + env(safe-area-inset-bottom, 0px))', borderTop: '0.5px solid var(--border)', background: 'var(--bg-surface)', flexShrink: 0 }}>
                ...
                <div className="chat-composer-row" style={{ display: 'flex', gap: 8, marginBottom: 10, position: 'relative', alignItems: 'flex-end' }}>
                  {/* Emoji picker popup */}
                  {showEmoji && (
              ...
              <div className="nx-card" style={{ width: '100%', maxWidth: 440 }}>
                ...
                <div style={{ padding: '1.25rem 1.5rem', display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  <div>
                    ...
                    <input className="nx-input" placeholder="Ex: Clínica CliniSac"
